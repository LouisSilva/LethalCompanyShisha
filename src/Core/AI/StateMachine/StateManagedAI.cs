...
public abstract class StateManagedAI<TState, TEnemyAI> : BaseAI
    where TState : Enum
        ...
        public static void InitializeIfNeeded()
        {
            ...
            lock (typeof(StateCache)) // Ensures thread safety during initialization
            {
                ...
                for (int i = 0; i < cachedAssemblies.Count; i++)
                {
                    ...
                    {
                        ShishaPlugin.Logger.LogError($"[StateCache<{typeof(TEnemyAI).Name}>] Error processing assembly {assembly.FullName}: {e}");
                    }
                }

                ShishaPlugin.LogVerbose($"[StateCache<{typeof(TEnemyAI).Name}>] Initialized with {StateTypes.Count} states.");
                IsInitialized = true;
            }
        }
    }
