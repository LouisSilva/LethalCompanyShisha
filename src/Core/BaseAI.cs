...
public abstract class BaseAI : EnemyAI
{
    ...
    public override void OnNetworkSpawn()
    {
        base.OnNetworkSpawn();
        if (!IsServer) return;
        _networkId.Value = new FixedString32Bytes(Guid.NewGuid().ToString("N").Substring(0, 8));
    }
    ...
    #region Logging
    internal void LogInfo(object message) => ShishaPlugin.Logger?.LogInfo($"{GetLogPrefix()} {message}");

    internal void LogVerbose(object message)
    {
        ShishaPlugin.Logger.LogDebug($"{GetLogPrefix()} {message}");
    }

    internal void LogDebug(object message) => ShishaPlugin.Logger.LogDebug($"{GetLogPrefix()} {message}");

    internal void LogError(object message) => ShishaPlugin.Logger.LogError($"{GetLogPrefix()} {message}");

    internal void LogWarning(object message) => ShishaPlugin.Logger.LogWarning($"{GetLogPrefix()} {message}");
